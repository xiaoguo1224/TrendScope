...
async function saveDefaults(): Promise<void> {
  saving.value = true
  try {
    const savedRanking = await Promise.all([updateSetting('collection_defaults', { ...collection }, 'Default collection parameters'), updateSetting('browser_defaults', { ...browser }, 'Default browser parameters'), ranking.id ? updateRankingConfig(ranking.id, ranking) : createRankingConfig(ranking)])
    if (!ranking.id) Object.assign(ranking, savedRanking[2])
...
function addProvider(): void { providers.value.push({ name: '', provider_type: 'llm', base_url: null, model_name: null, api_key: '', savedKeyMask: null, timeout_seconds: 60, max_retries: 2, enabled: false }) }
function addQueryExpansionTemplate(): void { templates.value.push({ name: 'query-expansion', purpose: 'query_expansion', template: 'Expand the user-provided keywords. Preserve all original keywords and return categorized additions only.', enabled: true }) }
...
<template>
  <section v-loading="loading" class="page-card settings-page">
    ...
    <el-tabs type="border-card">
      ...
      <el-tab-pane label="LLM Provider"><el-alert type="info" :closable="false" title="读取已有 Provider 时 API Key 只显示掩码；留空保存不会覆盖已有密钥。" /><el-button class="add-button" @click="addProvider">新增 Provider</el-button><el-card v-for="item in providers" :key="item.id ?? item.name" class="config-card"><el-form label-width="150px"><el-form-item label="名称"><el-input v-model="item.name" /></el-form-item><el-form-item label="类型"><el-select v-model="item.provider_type"><el-option value="llm" label="LLM" /><el-option value="vision" label="Vision" /></el-select></el-form-item><el-form-item label="Base URL"><el-input v-model="item.base_url" /></el-form-item><el-form-item label="模型"><el-input v-model="item.model_name" /></el-form-item><el-form-item label="API Key"><el-input v-model="item.api_key" show-password :placeholder="item.savedKeyMask ? `已保存：${item.savedKeyMask}；留空不覆盖` : '请输入 API Key'" /></el-form-item><el-form-item label="超时 (秒)"><el-input-number v-model="item.timeout_seconds" :min="1" :max="600" /></el-form-item><el-form-item label="最大重试次数"><el-input-number v-model="item.max_retries" :min="0" :max="10" /></el-form-item><el-form-item label="启用"><el-switch v-model="item.enabled" /></el-form-item><el-button type="primary" @click="saveProvider(item)">保存</el-button><el-button @click="remove(item, 'provider')">删除</el-button></el-form></el-card></el-tab-pane>
      <el-tab-pane label="Query Expansion Prompt"><el-alert type="info" :closable="false" title="Query Expansion 使用 purpose = query_expansion 的已启用模板。" /><el-button class="add-button" @click="addQueryExpansionTemplate">新增 Query Expansion 模板</el-button><el-card v-for="item in templates" :key="item.id ?? item.name" class="config-card"><el-form label-width="150px"><el-form-item label="名称"><el-input v-model="item.name" /></el-form-item><el-form-item label="用途"><el-input v-model="item.purpose" placeholder="query_expansion" /></el-form-item><el-form-item label="模板"><el-input v-model="item.template" type="textarea" :rows="7" /></el-form-item><el-form-item label="启用"><el-switch v-model="item.enabled" /></el-form-item><el-button type="primary" @click="saveTemplate(item)">保存</el-button><el-button @click="remove(item, 'template')">删除</el-button></el-form></el-card></el-tab-pane>
    </el-tabs>
  </section>
</template>
