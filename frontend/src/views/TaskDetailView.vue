...
const expandedKeywordGroups = computed(() => Object.entries(task.value?.expanded_keywords ?? {}).filter(([, values]) => values.length > 0))
  ...
  <section v-loading="loading" class="task-detail">
    ...
    <template v-if="task">
      ...
      <el-tabs v-model="activeTab" type="border-card" @tab-change="onTabChange">
        <el-tab-pane label="概览" name="contents"><el-card class="keyword-card" shadow="never"><template #header>扩展关键词</template><el-empty v-if="expandedKeywordGroups.length === 0" description="尚未完成扩词；开始研究后会在这里展示并保存当前任务的扩词结果。" /><el-descriptions v-else :column="1" border><el-descriptions-item v-for="[category, keywords] in expandedKeywordGroups" :key="category" :label="category"><el-tag v-for="keyword in keywords" :key="keyword" class="tag">{{ keyword }}</el-tag></el-descriptions-item></el-descriptions></el-card><el-table :data="contents" empty-text="尚未采集到公开内容"><el-table-column label="内容" min-width="360"><template #default="{ row }: { row: ContentItem }"><a :href="row.url" target="_blank" rel="noopener noreferrer">{{ row.title || row.text || row.external_id }}</a></template></el-table-column><el-table-column label="点赞" width="100" prop="like_count" /><el-table-column label="收藏" width="100" prop="favorite_count" /><el-table-column label="评论" width="100" prop="comment_count" /></el-table></el-tab-pane>
...
<style scoped>
.task-detail { max-width: 1280px; }.actions { display: flex; flex-wrap: wrap; gap: 12px; }.summary-card, .keyword-card, .concept-card, .prompt-card { margin-bottom: 16px; }.top-alert { margin-top: 16px; }.tag, .tags .el-tag { margin: 2px 6px 2px 0; }.tags { margin: 16px 0; }.report { white-space: pre-wrap; overflow: auto; line-height: 1.6; }
</style>
