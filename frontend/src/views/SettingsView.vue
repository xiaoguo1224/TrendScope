...
import type { AIProviderCapabilities, AIProviderConfig, AIProviderConfigTestResult, BrowserConnectionTestResult, PlatformConfig, PlatformConfigTestResult, PromptTemplate, RankingConfig } from '@/types'
...
function defaultCapabilities(providerType: string): AIProviderCapabilities { return { text: true, vision: providerType === 'vision', structured_output: true, tools: false, streaming: false } }
function providerForEdit(item: AIProviderConfig): EditableProvider { return { ...item, protocol: item.protocol ?? 'auto', priority: item.priority ?? 100, capabilities: { ...defaultCapabilities(item.provider_type), ...item.capabilities }, savedKeyMask: item.api_key ?? null, api_key: '' } }
...
async function saveProvider(item: EditableProvider): Promise<void> {
  const payload: AIProviderConfig = { id: item.id, name: item.name, provider_type: item.provider_type, base_url: item.base_url || null, model_name: item.model_name || null, api_key: item.api_key || null, protocol: item.protocol, capabilities: item.capabilities, priority: item.priority, timeout_seconds: item.timeout_seconds, max_retries: item.max_retries, enabled: item.enabled }
...
function addProvider(providerType: 'llm' | 'vision'): void { providers.value.push({ name: '', provider_type: providerType, base_url: null, model_name: null, api_key: '', savedKeyMask: null, protocol: 'auto', capabilities: defaultCapabilities(providerType), priority: 100, timeout_seconds: 60, max_retries: 2, enabled: false }) }
  ...
  <section v-loading="loading" class="page-card settings-page">
    ...
    <el-tabs type="border-card">
      ...
      <el-tab-pane label="AI Provider">
        <el-alert type="info" :closable="false" title="业务 Agent 只调用 Model Gateway；这里声明 Provider 协议、模型能力与路由优先级。URL 只作为该 Provider 的连接地址，不再决定业务逻辑。相同用途按优先级从小到大尝试；仅遇到限流、超时、网络或服务端故障时才切换到下一项。API Key 只以掩码回显。" />
        <el-divider content-position="left">LLM Provider</el-divider><el-button class="add-button" @click="addProvider('llm')">新增 LLM Provider</el-button><el-card v-for="item in llmProviders" :key="item.id ?? item.name" class="config-card"><el-form label-width="150px"><el-form-item label="名称"><el-input v-model="item.name" /></el-form-item><el-form-item label="Base URL"><el-input v-model="item.base_url" placeholder="例如 https://example.com/v1" /></el-form-item><el-form-item label="协议"><el-select v-model="item.protocol"><el-option label="自动兼容探测" value="auto" /><el-option label="OpenAI Responses" value="openai_responses" /><el-option label="OpenAI Chat Completions" value="openai_chat" /><el-option label="Anthropic Messages" value="anthropic_messages" /><el-option label="Gemini Generate Content" value="gemini" /><el-option label="Ollama Generate" value="ollama_generate" /><el-option label="Ollama Chat" value="ollama_chat" /></el-select></el-form-item><el-form-item label="模型"><el-input v-model="item.model_name" /></el-form-item><el-form-item label="路由优先级"><el-input-number v-model="item.priority" :min="0" :max="10000" /><div class="form-help">数值越小越优先；同用途的下一项作为可恢复故障备用。</div></el-form-item><el-form-item label="模型能力"><el-checkbox v-model="item.capabilities.text">文本</el-checkbox><el-checkbox v-model="item.capabilities.vision">识图</el-checkbox><el-checkbox v-model="item.capabilities.structured_output">结构化输出</el-checkbox><el-checkbox v-model="item.capabilities.tools">工具调用</el-checkbox><el-checkbox v-model="item.capabilities.streaming">流式</el-checkbox></el-form-item><el-form-item label="API Key"><el-input v-model="item.api_key" show-password :placeholder="item.savedKeyMask ? `已保存：${item.savedKeyMask}；留空不覆盖` : '请输入 API Key'" /></el-form-item><el-form-item label="超时 (秒)"><el-input-number v-model="item.timeout_seconds" :min="1" :max="600" /></el-form-item><el-form-item label="最大重试次数"><el-input-number v-model="item.max_retries" :min="0" :max="10" /></el-form-item><el-form-item label="启用"><el-switch v-model="item.enabled" /></el-form-item><el-button type="primary" @click="saveProvider(item)">保存</el-button><el-button :loading="providerTesting === item.id" @click="runProviderTest(item)">测试模型配置</el-button><el-button @click="remove(item, 'provider')">删除</el-button><el-alert v-if="item.id && providerTestResults[item.id]" class="provider-test-result" :type="providerTestResults[item.id].success ? 'success' : 'error'" :closable="false" :title="providerTestResults[item.id].message" :description="providerTestDescription(providerTestResults[item.id])" /></el-form></el-card>
        <el-divider content-position="left">Vision Provider</el-divider><el-button class="add-button" @click="addProvider('vision')">新增 Vision Provider</el-button><el-card v-for="item in visionProviders" :key="item.id ?? item.name" class="config-card"><el-form label-width="150px"><el-form-item label="名称"><el-input v-model="item.name" /></el-form-item><el-form-item label="Base URL"><el-input v-model="item.base_url" placeholder="例如 https://example.com/v1" /></el-form-item><el-form-item label="协议"><el-select v-model="item.protocol"><el-option label="自动兼容探测" value="auto" /><el-option label="OpenAI Responses" value="openai_responses" /><el-option label="OpenAI Chat Completions" value="openai_chat" /><el-option label="Anthropic Messages" value="anthropic_messages" /><el-option label="Gemini Generate Content" value="gemini" /><el-option label="Ollama Generate" value="ollama_generate" /><el-option label="Ollama Chat" value="ollama_chat" /></el-select></el-form-item><el-form-item label="模型"><el-input v-model="item.model_name" /></el-form-item><el-form-item label="路由优先级"><el-input-number v-model="item.priority" :min="0" :max="10000" /><div class="form-help">数值越小越优先；同用途的下一项作为可恢复故障备用。</div></el-form-item><el-form-item label="模型能力"><el-checkbox v-model="item.capabilities.text">文本</el-checkbox><el-checkbox v-model="item.capabilities.vision">识图</el-checkbox><el-checkbox v-model="item.capabilities.structured_output">结构化输出</el-checkbox><el-checkbox v-model="item.capabilities.tools">工具调用</el-checkbox><el-checkbox v-model="item.capabilities.streaming">流式</el-checkbox></el-form-item><el-form-item label="API Key"><el-input v-model="item.api_key" show-password :placeholder="item.savedKeyMask ? `已保存：${item.savedKeyMask}；留空不覆盖` : '请输入 API Key'" /></el-form-item><el-form-item label="超时 (秒)"><el-input-number v-model="item.timeout_seconds" :min="1" :max="600" /></el-form-item><el-form-item label="最大重试次数"><el-input-number v-model="item.max_retries" :min="0" :max="10" /></el-form-item><el-form-item label="启用"><el-switch v-model="item.enabled" /></el-form-item><el-button type="primary" @click="saveProvider(item)">保存</el-button><el-button :loading="providerTesting === item.id" @click="runProviderTest(item)">测试模型配置</el-button><el-button @click="remove(item, 'provider')">删除</el-button><el-alert v-if="item.id && providerTestResults[item.id]" class="provider-test-result" :type="providerTestResults[item.id].success ? 'success' : 'error'" :closable="false" :title="providerTestResults[item.id].message" :description="providerTestDescription(providerTestResults[item.id])" /></el-form></el-card>
      </el-tab-pane>
