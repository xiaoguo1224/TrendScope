...
import type { ContentItem, CreativeConcept, ImagePrompt, RankingItem, ResearchTask, ResearchTaskCreate, TaskAnalysisResult, TaskRankings, TaskReport, TrendAnalysis } from '@/types'
...
// Collection includes real browser navigation, scrolling and public-media
// downloads. Its request budget must not be shorter than browser timeout.
export const runResearchTask = async (id: number): Promise<ResearchTask> => (
  await client.post(`/research/tasks/${id}/run`, null, { timeout: 15 * 60_000 })
).data
...
export const getTaskAnalysis = async (id: number): Promise<TaskAnalysisResult> => {
  return (await client.get(`/research/tasks/${id}/analysis`)).data as TaskAnalysisResult
}

export const runTaskAnalysis = async (id: number, regenerate = false): Promise<TaskAnalysisResult> => (
  await client.post(`/research/tasks/${id}/analysis/run`, null, { params: { regenerate }, timeout: 5 * 60_000 })
).data as TaskAnalysisResult
